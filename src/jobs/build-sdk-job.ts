...
export class BuildSdkJob {
  static id = 'build_sdk';

  static render(options: BuildSdkJobOptions): workflows.Job {
    ...
    const steps: JobStep[] = [
      ...
      ...ProviderActions.setupTools({
        tools: ['pulumictl', 'pulumicli', ...languages.map((lang) => lang)],
      }),
      {
      ...
      ...ProviderActions.uploadSdk('${{ matrix.language }}'),
    ];
    if (options.freeDiskSpaceBeforeBuild) {
      steps.unshift(ProviderWorkflowSteps.freeDiskSpace());
    }
    ...
    return {
      ...
      strategy: {
        matrix,
        failFast: "${{ ! contains(github.actor, 'renovate')}}" as any,
      },
